quest events_mgr begin
	state start begin
		when letter with pc.is_gm() begin
			local lang2 = character.get_language()
			send_letter(events_mgr_text[lang2][0])
		end
		
		when button or info with pc.is_gm() begin
			local lang2 = character.get_language()
			local lang = character.get_language() + 100
			say_title(events_mgr_text[lang2][0])
			say("")
			local events = select(events_mgr_text[lang][0], events_mgr_text[lang][2])
			if events == 2 then
				send_letter(events_mgr_text[lang2][0])
				return
			elseif events == 1 then
				say_title(events_mgr_text[lang][0])
				say("")
				local menu = select(events_mgr_text[lang][1], events_mgr_text[lang][4], events_mgr_text[lang][2])
				if menu == 3 then
					send_letter(events_mgr_text[lang2][0])
					return
				elseif menu == 1 then
					...
					if pc.get_channel_id() != kingdoms_war.get_map_ch() then
						say(string.format(events_mgr_text[lang][3], kingdoms_war.get_map_ch()))
						wait()
						send_letter(events_mgr_text[lang2][0])
						return
					end
					
					say(events_mgr_text[lang][10])
					local submenu = select(events_mgr_text[lang][11], events_mgr_text[lang][12])
					say_title(events_mgr_text[lang][0])
					say("")
					if submenu == 2 then
						say(events_mgr_text[lang][20])
						...
						if min_lv > 0 then
							say(string.format(events_mgr_text[lang][21], min_lv))
						else
							say(events_mgr_text[lang][22])
						end
						
						if max_lv > 0 then
							say(string.format(events_mgr_text[lang][23], max_lv))
						else
							say(events_mgr_text[lang][24])
						end
						
						if r_yang > 0 then
							say(string.format(events_mgr_text[lang][32], r_yang))
						else
							say(events_mgr_text[lang][33])
						end
						
						if r_count > 0 and r_item > 0 then
							say(string.format(events_mgr_text[lang][34], r_count, item_name(r_item)))
						else
							say(events_mgr_text[lang][35])
						end
						
						if t_yang > 0 then
							say(string.format(events_mgr_text[lang][36], t_yang))
						else
							say(events_mgr_text[lang][37])
						end
						
						if t_count > 0 and t_item > 0 then
							say(string.format(events_mgr_text[lang][38], t_count, item_name(t_item)))
						else
							say(events_mgr_text[lang][39])
						end
						
						wait()
						say_title(events_mgr_text[lang][0])
						say("")
						say(events_mgr_text[lang][25])
						local submenu_agree = select(events_mgr_text[lang][11], events_mgr_text[lang][12])
						...
						say(events_mgr_text[lang][15])
						local min_lv = tonumber(input())
						say_title(events_mgr_text[lang][0])
						say("")
						if not min_lv or min_lv == nil or min_lv == "" then
							say(events_mgr_text[lang][17])
							wait()
							send_letter(events_mgr_text[lang2][0])
							return
						end
						
						say(events_mgr_text[lang][16])
						...
						if not kingdoms_war.config(min_lv, max_lv, r_yang, r_itemVnum, r_itemCount, t_yang, t_itemVnum, t_itemCount) then
							say(events_mgr_text[lang][19])
							wait()
							send_letter(events_mgr_text[lang2][0])
							return
						else
							say(events_mgr_text[lang][18])
							wait()
							send_letter(events_mgr_text[lang2][0])
							return
						end
